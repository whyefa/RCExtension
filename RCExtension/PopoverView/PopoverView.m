...
static CGFloat const kPopoverViewArrowHeight = 10.f;  ///< 箭头高度
...
- (void)setBorderStyle:(PopoverViewBorderStyle)borderStyle {
    _borderStyle = borderStyle;
    _shadeView.backgroundColor = (borderStyle == PopoverViewBorderStyleDark) ? [UIColor colorWithWhite:0.f alpha:0.18f] : [UIColor clearColor];
    if (_borderLayer) {
        _borderLayer.strokeColor = (borderStyle != PopoverViewBorderStyleBoarder) ? [UIColor clearColor].CGColor : _tableView.separatorColor.CGColor;
    }
}
...
- (void)initialize {
    ...
    _borderStyle = PopoverViewBorderStyleDark;
...
- (void)showToPoint:(CGPoint)toPoint {
    ...
    // 边框样式
    if (self.borderStyle == PopoverViewBorderStyleBoarder) {
    ...
    if (self.borderStyle == PopoverViewBorderStyleShadow) {
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        _tableView.separatorColor = [PopoverViewCell bottomLineColorForStyle: PopoverViewStyleDefault];
